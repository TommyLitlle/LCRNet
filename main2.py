...
# classes = 10
data_path = r'/data/home/zhangxiaoqing/Zhangxq/liver_ct'
...
trainset = dataset(os.path.join(data_path, 'train'), transform=transform_train)
trainloader = DataLoader(
    trainset, batch_size=batch_size, shuffle=True, num_workers=8,drop_last=True)
vaildset = dataset(os.path.join(data_path, 'test'), transform=transform_vaild)
vaildset_add = dataset(os.path.join(data_path, 'test'), transform=transform_train)
...
def checkpoint(acc, epoch):
    ...
    if not os.path.isdir(os.path.join(sys.path[0], 'CT')):
        os.mkdir(os.path.join(sys.path[0], 'CT'))
    torch.save(state, os.path.join(sys.path[0], 'CT', model_name +
                                   config["train"]["sess"] + '_' + str(seed)))
...
def test():
    ...
    global best_acc
    global keys
    model_path = os.path.join(sys.path[0], 'CT', model_name + config["train"]["sess"] + '_' + str(seed))
    print(model_path)
    ...
    testset = dataset(os.path.join(data_path, 'test'), transform=transform_vaild)
